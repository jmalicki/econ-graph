...
describe('Chart API Server', () => {
  ...
  describe('GET /health', () => {
    it('should return health status with MCP server header', async () => {
      ...
      expect(response.body.message).toBe('Chart API Service is healthy');
  ...
  describe('GET /api/chart/types', () => {
    it('should return supported chart types', async () => {
      const response = await request(app)
        .get('/api/chart/types')
        .set('X-MCP-Server-Request', 'true')
        .set('X-Internal-Request', 'true');
      ...
      expect(response.status).toBe(200);
      expect(response.body.success).toBe(true);
      expect(response.body.chartTypes).toHaveLength(3);
      expect(response.body.chartTypes.map(t => t.type)).toEqual(['line', 'bar', 'scatter']);
    });

    it('should reject without proper headers', async () => {
      const response = await request(app)
        .get('/api/chart/types');
  ...
  describe('GET /api/info', () => {
    it('should return service information', async () => {
      ...
      expect(response.status).toBe(200);
      expect(response.body.success).toBe(true);
      expect(response.body.service).toBe('EconGraph Chart API');
      expect(response.body.version).toBe('1.0.0');
