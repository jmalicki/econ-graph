...
impl ComprehensiveCrawlerScheduler {
    ...
    async fn update_data_source_crawl_status(&self) -> AppResult<()> {
        ...
        diesel::update(
            econ_graph_core::schema::data_sources::table
                .filter(econ_graph_core::schema::data_sources::is_enabled.eq(true)),
        )
        .set((
            econ_graph_core::schema::data_sources::last_crawl_at.eq(Some(chrono::Utc::now())),
            econ_graph_core::schema::data_sources::crawl_status.eq(Some("completed".to_string())),
            econ_graph_core::schema::data_sources::crawl_error_message.eq(None::<String>),
            econ_graph_core::schema::data_sources::updated_at.eq(chrono::Utc::now()),
        ))
        .execute(&mut conn)
        .await?;
