...
use econ_graph_core::models::{DataSource, SeriesMetadata, NewSeriesMetadata};
use reqwest::Client;
...
impl ApiDiscoveryService {
    ...
    pub async fn discover_series_for_source(
        ...
        let config = self.configs.get(source_name)
            .ok_or_else(|| econ_graph_core::error::AppError::ConfigurationError(
    ...
    async fn discover_from_endpoint(
        ...
        if !response.status().is_success() {
            return Err(econ_graph_core::error::AppError::ExternalApiError(
    ...
    fn parse_series_from_response(
        ...
        let series_array = json.pointer(&format!("/{}", config.series_path))
            ...
            .ok_or_else(|| econ_graph_core::error::AppError::ExternalApiError(
