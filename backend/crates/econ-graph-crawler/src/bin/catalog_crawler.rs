...
use econ_graph_core::database::{create_pool, DatabasePool};
use econ_graph_core::models::data_source::DataSource;
use econ_graph_services::services::crawler::catalog_downloader::CatalogDownloader;
use econ_graph_services::services::crawler::series_downloader::SeriesDownloader;
use econ_graph_services::services::series_discovery::SeriesDiscoveryService;
use reqwest::Client;
...
async fn export_catalog_to_sql(
    ...
    let data_sources = DataSource::find_all(&pool).await?;
    let series_metadata =
        econ_graph_core::models::series_metadata::SeriesMetadata::find_all(&pool).await?;
    let economic_series =
        econ_graph_core::models::economic_series::EconomicSeries::find_all(&pool).await?;
    let data_points = econ_graph_core::models::data_point::DataPoint::find_all(&pool).await?;
    let crawl_attempts =
        econ_graph_core::models::crawl_attempt::CrawlAttempt::find_all(&pool).await?;
