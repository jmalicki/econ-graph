...
impl AuthService {
    ...
    pub async fn create_or_update_oauth_user(
        ...
        let db_user = econ_graph_core::models::User::create_or_get_oauth(
            &self.db_pool,
    ...
    pub async fn create_email_user(
        ...
        let db_user =
            econ_graph_core::models::User::create_with_email(&self.db_pool, email, password, name)
                .await?;
        Ok(db_user.to_auth_user())
    }
    ...
    pub async fn authenticate_email_user(
        ...
        let db_user =
            econ_graph_core::models::User::authenticate(&self.db_pool, email, password).await?;
        Ok(db_user.to_auth_user())
    }

    /// Get user by ID using actual database lookup
    pub async fn get_user_by_id(&self, user_id: Uuid) -> AppResult<Option<User>> {
        match econ_graph_core::models::User::get_by_id(&self.db_pool, user_id).await {
    ...
    pub async fn update_user_profile(
        ...
        let db_updates = econ_graph_core::models::user::UpdateUser {
            name: updates.name,
        ...
        // Use the actual database User model method and convert to auth User
        let db_user =
            econ_graph_core::models::User::update_profile(&self.db_pool, user_id, db_updates)
                .await?;
        Ok(db_user.to_auth_user())
    }
