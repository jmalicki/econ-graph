...
use crate::models::{DataSource, NewDataSource, EconomicSeries, NewEconomicSeries, DataPoint, NewDataPoint};
...
async fn test_complete_graphql_workflow() {
    ...
    // Create test data
    let test_source = DataSource::create(&pool, NewDataSource {
    ...
    }).await.expect("Should create test data source");
    
    let test_series = EconomicSeries::create(&pool, &NewEconomicSeries {
        ...
        is_active: true,
    }).await.expect("Should create test series");
    
    // Create test data points
    let test_data: Vec<NewDataPoint> = (1..=12).map(|month| {
        NewDataPoint {
    ...
    DataPoint::create_batch(&pool, &test_data).await
    ...
    let series_from_db = EconomicSeries::find_by_external_id(&pool, "E2E_TEST_001", test_source.id).await
    ...
    ).await.expect("Should retrieve data points");
    ...
    let yoy_change = ((last_point_value - first_point_value) / first_point_value) * BigDecimal::from(100);
...
async fn test_crawler_monitoring_workflow() {
    ...
    let initial_stats = crate::services::queue_service::get_queue_statistics(&pool).await
    ...
    for item in queue_items {
        CrawlQueueItem::create(&pool, &item).await
            .expect("Should create queue item");
    }
    
    // Test updated statistics (admin dashboard would poll this)
    let updated_stats = crate::services::queue_service::get_queue_statistics(&pool).await
    ...
    let next_item = crate::services::queue_service::get_and_lock_next_item(&pool, "e2e-worker").await
    ...
    let processing_stats = crate::services::queue_service::get_queue_statistics(&pool).await
    ...
    crate::services::queue_service::mark_item_completed(&pool, locked_item.id).await
        .expect("Should complete queue item");
    
    // Test final statistics (admin dashboard completion tracking)
    let final_stats = crate::services::queue_service::get_queue_statistics(&pool).await
    ...
    let crawler_status = crate::services::crawler_service_simple::get_crawler_status().await
...
async fn test_search_integration_workflow() {
    ...
    let test_source = DataSource::create(&pool, NewDataSource {
        ...
        api_key_required: false,
        rate_limit_per_minute: 1000,
    }).await.expect("Should create search test source");
    
    // Create multiple series with searchable content
    let search_series = vec![
        ("GDP_SEARCH_001", "Gross Domestic Product - Real", "Real GDP measures economic output"),
        ("UNEMPLOYMENT_002", "Unemployment Rate - National", "National unemployment statistics"),
        ("INFLATION_003", "Consumer Price Index", "Inflation measurement via CPI"),
        ("HOUSING_004", "Housing Price Index", "Residential real estate pricing"),
    ];
    
    let mut created_series = Vec::new();
    
    for (external_id, title, description) in search_series {
        let series = EconomicSeries::create(&pool, &NewEconomicSeries {
            source_id: test_source.id,
            ...
            end_date: Some(NaiveDate::from_ymd_opt(2024, 12, 31).unwrap()),
            is_active: true,
        }).await.expect("Should create search test series");
...
async fn test_data_transformation_workflow() {
    ...
    let test_source = DataSource::create(&pool, NewDataSource {
        ...
        api_key_required: false,
        rate_limit_per_minute: 1000,
    }).await.expect("Should create transform test source");
    ...
    let test_series = EconomicSeries::create(&pool, &NewEconomicSeries {
        ...
        end_date: Some(NaiveDate::from_ymd_opt(2024, 12, 31).unwrap()),
        is_active: true,
    }).await.expect("Should create transform test series");
    ...
    let all_data_points = DataPoint::find_by_series_and_date_range(
        ...
        NaiveDate::from_ymd_opt(2024, 12, 31).unwrap()
    ).await.expect("Should retrieve all data points");
    ...
    ).await.expect("Should apply YoY transformation");
    ...
    for (i, point) in yoy_transformed.iter().enumerate() {
        if i >= 12 { // Second year points should have YoY values
    ...
    ).await.expect("Should apply MoM transformation");
    ...
    ).await.expect("Should apply percent change transformation");
