...
pub async fn unlock_stuck_items(
    pool: &DatabasePool,
    timeout_minutes: i64,
) -> AppResult<i64> {
...
mod tests {
    ...
    async fn test_queue_item_locking_and_unlocking() {
        ...
        // Lock the item
        lock_queue_item(&pool, created_item.id, worker_id).await.unwrap();
        
        // Verify item is locked (should not appear in next items)
        let items = get_next_queue_items(&pool, 10).await.unwrap();
        assert_eq!(items.len(), 0, "Locked item should not appear in available items");
    ...
    async fn test_queue_retry_logic() {
        ...
        update_queue_item_for_retry(
            &pool, 
            created_item.id, 
            Some("Still timing out".to_string())
        ).await.unwrap();
    ...
    async fn test_cleanup_old_queue_items() {
        ...
        let deleted_count = cleanup_old_queue_items_with_retention(&pool, 0).await.unwrap();
