...
impl GlobalAnalysisService {
    ...
    pub async fn get_countries_with_economic_data(
        ...
        for country in countries_data {
            ...
            result.push(CountryWithEconomicData {
                country,
                latest_gdp: economic_data.get("GDP").cloned(),
                latest_gdp_growth: economic_data.get("GDP_GROWTH").cloned(),
                latest_inflation: economic_data.get("INFLATION").cloned(),
                latest_unemployment: economic_data.get("UNEMPLOYMENT").cloned(),
                trade_partners,
    ...
    pub async fn calculate_country_correlations(
        ...
        for correlation in &correlations {
            ...
            diesel::insert_into(country_correlations::table)
                ...
                .do_update()
                .set((
                    country_correlations::correlation_coefficient.eq(new_correlation.correlation_coefficient),
                    country_correlations::sample_size.eq(new_correlation.sample_size),
                    country_correlations::p_value.eq(new_correlation.p_value),
    ...
    pub async fn get_correlation_network(
        ...
        let correlations = country_correlations
            ...
            .filter(correlation_coefficient.ge(Decimal::from_f64_retain(min_correlation).unwrap_or_default()))
        ...
        for correlation in &correlations {
            if !countries_map.contains_key(&correlation.country_a_id) {
            ...
            if !countries_map.contains_key(&correlation.country_b_id) {
                let country = countries::table
            ...
            let connection_a_to_b = CorrelationConnection {
                ...
                indicator_category: correlation.indicator_category.clone(),
                significance_level: correlation.p_value.map(|p| p.to_f64().unwrap_or(1.0)).unwrap_or(1.0),
                connection_strength,
            };
    ...
    pub async fn get_global_events_with_impacts(
        ...
        for event in events {
            ...
            let total_economic_impact = event.economic_impact_score;
    ...
    async fn get_top_trade_partners(
        ...
        for trade in trade_data {
            ...
            let trade_intensity = trade.trade_intensity.map(|t| t.to_f64().unwrap_or(0.0)).unwrap_or(0.0);
    ...
    async fn get_latest_economic_indicators(
        ...
        for indicator in indicators {
            if let Ok(latest_data) = global_indicator_data
                ...
                .await
            {
                result.insert(indicator.category.clone(), latest_data.value.unwrap_or_default());
            }
        }
    ...
    async fn calculate_pairwise_correlation(
        ...
        Ok(CountryCorrelation {
            ...
            correlation_coefficient: Decimal::from_f64_retain(0.75).unwrap_or_default(), // Mock correlation
            time_period_start: start_date,
            time_period_end: end_date,
            sample_size: 100,
            p_value: Some(Decimal::from_f64_retain(0.01).unwrap_or_default()),
            is_significant: true,
    ...
    fn classify_impact_severity(impact: &crate::models::EventCountryImpact) -> String {
        match impact.impact_magnitude.map(|m| m.to_f64().unwrap_or(0.0).abs()) {
