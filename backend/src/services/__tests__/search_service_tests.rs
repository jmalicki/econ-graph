...
mod tests {
    ...
    async fn create_test_search_data(pool: &DatabasePool) -> AppResult<(Uuid, Vec<Uuid>)> {
        ...
        use crate::models::{NewDataSource, NewEconomicSeries};
        ...
        let source = diesel::insert_into(data_sources::table)
            .values(&new_source)
            .returning(crate::models::DataSource::as_select())
            .get_result::<crate::models::DataSource>(&mut conn)
            .await?;
        ...
        for series in test_series {
            let created_series = diesel::insert_into(economic_series::table)
                .values(&series)
                .returning(crate::models::EconomicSeries::as_select())
                .get_result::<crate::models::EconomicSeries>(&mut conn)
                .await?;
