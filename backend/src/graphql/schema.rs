...
mod tests {
    ...
    async fn test_schema_creation() {
        ...
        assert!(result.errors.is_empty(), "Introspection query should execute without errors");
        
        let data = result.data.into_json().unwrap();
        // Verify Query root type is properly configured - required for read operations
        assert_eq!(data["__schema"]["queryType"]["name"], "Query", "Schema should have Query root type");
        // Verify Mutation root type is properly configured - required for write operations
        assert_eq!(data["__schema"]["mutationType"]["name"], "Mutation", "Schema should have Mutation root type");
    }
}
