...
# Count frontend test code
FRONTEND_TESTS_FILES=$(find ./frontend -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" 2>/dev/null | grep test | wc -l)
FRONTEND_TESTS_LINES=$(find ./frontend -name "*.ts" -o -name "*.tsx" -o -name "*.js" -o -name "*.jsx" 2>/dev/null | grep test | xargs wc -l 2>/dev/null | tail -1 | awk '{print $1}' || echo "0")
...
- **Frontend Development**: \$$(printf "%'d" $(echo "scale=0; $FRONTEND_PROD_COST + $FRONTEND_TESTS_COST" | bc)) ($(echo "scale=1; ($FRONTEND_PROD_COST + $FRONTEND_TESTS_COST) * 100 / $TOTAL_COST" | bc)%)
- **Backend Development**: \$$(printf "%'d" $(echo "scale=0; $BACKEND_PROD_COST + $BACKEND_TESTS_COST" | bc)) ($(echo "scale=1; ($BACKEND_PROD_COST + $BACKEND_TESTS_COST) * 100 / $TOTAL_COST" | bc)%)
- **Infrastructure/DevOps**: \$$(printf "%'d" $(echo "scale=0; $CONFIG_COST + $DEPLOYMENT_COST" | bc)) ($(echo "scale=1; ($CONFIG_COST + $DEPLOYMENT_COST) * 100 / $TOTAL_COST" | bc)%)
...
**Traditional Development Cost**: \$$(printf "%'d" $TOTAL_COST)
**AI-Assisted Development Cost**: \$50,000 - \$100,000 (estimated)
**Time Savings**: 10-20x faster development cycles
**ROI**: $(echo "scale=0; $TOTAL_COST / 100000" | bc)-$(echo "scale=0; $TOTAL_COST / 50000" | bc)% return on AI investment
